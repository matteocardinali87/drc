...
void createRootGeometry_DIRC_prova(Bool_t sepEV = kTRUE, Int_t fFocusingSystem = 1, Bool_t fprizm = kFALSE){
...
/*
  ...
  cout<<"bar width = "<<barWidth/2. << ", bar with gaps = "<<(bbX/barnum)<<endl;
  ...
  // create logic bar:  
  TGeoBBox* logicBar = new TGeoBBox("logicBar",  barWidth/2., hthick, bbox_hlen-fabs(len)/2.-mirr_hthick);
  TGeoVolume* bar;
  if(fprizm == kFALSE){  
  ...
  // create logic mirror:
  TGeoBBox* logicMirror  = new TGeoBBox("logicMirror", barWidth/2., hthick, mirr_hthick);
