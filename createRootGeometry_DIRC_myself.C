...
void createRootGeometry_DIRC_myself(Bool_t sepEV = kTRUE, Int_t fFocusingSystem = 0){
  ...
  // Setup Mainz (test beam)
    ...
    //MCP parameters
    Double_t gluethickness=0.01; //[cm]
    Double_t mcpthickness=0.1; // [cm]
...
*/			    
 ...
 //--------------------------------------------------------------------------------------------- 
  ...
  gGeoManager->SetTopVolume(cave);
    ...
    vLocalMother = new TGeoVolume("BarrelDIRCShape", shape, air_m);
    ...
    /*
     ...
     |          |       |____________________________________| bar_height
     |          | air_gap
     |          |
     |__________|
    ...
    TGeoVolume* abox;
    //overlaps 1
    TGeoBBox*   logicBarAir = new TGeoBBox("logicBarAir",bar_height/2.+0.1,bar_width/2.+0.1,bar_lenght/2.); //Air aroud bar
    TGeoBBox*   logicEVAir= new TGeoBBox("logicEVAir",EV_width/2.+0.1,EV_height/2.+0.1,EV_lenght/2.+air_gap/2);//Air around EV and airgap
    //TGeoBBox*   logicBarAir = new TGeoBBox("logicBarAir",bar_height/2.+0.01,bar_width/2.+0.01,bar_lenght/2.);
    //TGeoBBox*   logicEVAir= new TGeoBBox("logicEVAir",EV_width/2.,EV_height/2.,EV_lenght/2.+0.1+air_gap/2+gluethickness/2+mcpthickness/2);
   
    
    TGeoTranslation* trseb = new TGeoTranslation("trseb", 0.,+shift_bar_EV/2.,-bar_lenght/2.-EV_lenght/2-air_gap/2);//translate EV Air
   ...
   /*
    // PhotoDetector:
    TGeoBBox *logicMcp = new TGeoBBox("logicMcp",EV_width/2.,EV_height/2.,mcpthickness/2.) ; //same XY dimension of EV
    TGeoVolume* Mcp = new TGeoVolume("DrcPDSensor", logicMcp, FusedSil_m);
    Mcp->SetLineColor(kGreen+3);
    
    TGeoBBox *logicMcpGlue = new TGeoBBox("logicMcpGlue",EV_width/2.,EV_height/2.,gluethickness/2. );//same XY dimension of EV
    TGeoVolume* McpGlue = new TGeoVolume("DrcMcpGlue", logicMcpGlue, OpticalGrease_m);
    McpGlue->SetLineColor(4);//(kCyan);//-9);
    McpGlue->SetTransparency(40);
    
    abox->AddNode(McpGlue, 1, new TGeoCombiTrans(dx,dy+shift_bar_EV/2.,-bar_lenght/2.-EV_lenght-air_gap-gluethickness/2.,new TGeoRotation(0)));
    abox->AddNode(Mcp, 1, new TGeoCombiTrans(dx,dy+shift_bar_EV/2.,-bar_lenght/2.-EV_lenght-air_gap-gluethickness-mcpthickness/2.+0.1,new TGeoRotation(0)));
